...
class LaneDetection:
    ...
    def cut_gray(self, state_image_full):
        ...
        '''
        gray_state_image = state_image_full[:self.cut_size]

        gray_state_image = np.mean(gray_state_image, axis=2, keepdims=True)
    ...
    def edge_detection(self, gray_image):
        ...
        '''
        gray_image = np.reshape(gray_image, (self.cut_size, 96))
        gradient_sum = np.abs(np.gradient(gray_image))
    ...
    def find_maxima_gradient_rowwise(self, gradient_sum):
        ...
        for i, row in enumerate(gradient_sum):
            peaks, _ = find_peaks(row, distance=self.distance_maxima_gradient)
            if peaks.size > 0:
                argmaxima.append(np.column_stack([peaks, np.full_like(peaks, i)]))

        if len(argmaxima) > 0:
            return np.vstack(argmaxima)
        else:
            return np.array([])
    ...
    def lane_detection(self, state_image_full):
        ...
        if lane_found:
            ...
            while maxima.shape[0] > 0:
                last_point1 = lane_boundary1_points[-1]
                dist1 = np.sum((maxima - last_point1)**2, axis=1)
                min_idx1 = np.argmin(dist1)
                min_dist1 = dist1[min_idx1]

                if min_dist1 < 100:
                    if min_dist1 > 0:
                        lane_boundary1_points = np.append(lane_boundary1_points, [maxima[min_idx1]], axis = 0)
                    maxima = np.delete(maxima, min_idx1, axis=0)
                else:
                    break
            ...
            while maxima.shape[0] > 0:
                last_point2 = lane_boundary2_points[-1]
                dist2 = np.sum((maxima - last_point2)**2, axis=1)
                min_idx2 = np.argmin(dist2)
                min_dist2 = dist2[min_idx2]

                if min_dist2 < 100:
                    if min_dist2 > 0:
                        lane_boundary2_points = np.append(lane_boundary2_points, [maxima[min_idx2]], axis = 0)
                    maxima = np.delete(maxima, min_idx2, axis=0)
                else:
                    break
            ...
            if lane_boundary1_points.shape[0] > 4 and lane_boundary2_points.shape[0] > 4:
                ...
                # lane_boundary 1
                lane_boundary1, _ = splprep(lane_boundary1_points.T, s=self.spline_smoothness)
                # lane_boundary 2
                lane_boundary2, _ = splprep(lane_boundary2_points.T, s=self.spline_smoothness)
